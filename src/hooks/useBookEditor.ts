 ...
 */
export function useBookEditor(projectId: string | undefined): UseBookEditorReturn {
  ...
  const stateRef = useRef({ selectedChapterId, selectedSectionId, selectedLocale, chapters, sections });
  useEffect(() => {
    stateRef.current = { selectedChapterId, selectedSectionId, selectedLocale, chapters, sections };
  });
  ...
  useEffect(() => {
    if (!projectId) return;
    setLoading(true);
    repository.getProject(projectId).then((p) => {
      setProject(p);
  ...
  orderedChapters.forEach((ch) => {
    orderedSections[ch._id] = sortByOrder(sections[ch._id] ?? [], ch.sectionOrder);
  });
  ...
  const addChapter = useCallback(async (type: ChapterType) => {
  ...
  const deleteChapter = useCallback(async (chapter: Chapter) => {
  ...
  const renameChapter = useCallback(async (chapter: Chapter, title: string) => {
    if (!projectId) return;
    await repository.saveChapter({ ...chapter, title });
    await loadChapters(projectId);
  }, [projectId, loadChapters]);

  const reorderChapters = useCallback(async (newIds: string[]) => {
    if (!project) return;
    const savedProject = await repository.saveProject({ ...project, chapterOrder: newIds });
    setProject(savedProject);
  }, [project]);

  // ─── Section operations ────────────────────────────────────────────────

  const addSection = useCallback(async (chapter: Chapter) => {
    if (!projectId) return;
    const now = new Date().toISOString();
  ...
  const deleteSection = useCallback(async (section: Section) => {
    if (!projectId) return;
  ...
  const renameSection = useCallback(async (section: Section, title: string) => {
    await repository.saveSection({ ...section, title });
    await loadSections(section.chapterId);
  }, [loadSections]);

  const reorderSections = useCallback(async (chapter: Chapter, newIds: string[]) => {
    if (!projectId) return;
  ...
  }, [loadChapters, loadSections, projectId]); // stateRef never changes identity
