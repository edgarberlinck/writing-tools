...
interface Props {
  ...
  onAddChapter: (type: ChapterType) => Promise<void> | void;
  ...
  return (
    <div className="w-64 bg-gray-800 flex flex-col h-full border-r border-gray-700 flex-shrink-0">
      ...
      <div className="p-3 border-t border-gray-700 relative">
        ...
        {showTypeMenu && (
          <div className="absolute bottom-full left-3 right-3 mb-1 bg-gray-700 rounded-lg shadow-lg overflow-hidden z-20 border border-gray-600">
            {CHAPTER_TYPES.map((t) => (
              <button
                key={t.value}
                onClick={async () => {
                  ...
                  try {
                    await props.onAddChapter(t.value);
                    setShowTypeMenu(false);
                  } catch (error) {
                    console.error("Failed to add chapter", error);
                    alert("Could not add chapter. Please try again.");
                  } finally {
