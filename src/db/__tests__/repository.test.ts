...
// ─── Minimal in-memory mock document store ───────────────────────────────────
...
describe("BookRepository", () => {
  ...
  describe("getProject", () => {
    ...
    it("throws when the id does not exist", async () => {
      await expect(repo.getProject("missing")).rejects.toMatchObject({
        status: 404,
      });
    });
  });
  ...
  describe("getChaptersByProject", () => {
    it("returns only chapters for the specified project", async () => {
      await repo.saveChapter(
        makeChapter({ _id: "ch_1", projectId: "project_1" }),
      );
      await repo.saveChapter(
        makeChapter({ _id: "ch_2", projectId: "project_2" }),
      );
  ...
  describe("getSectionsByChapter", () => {
    it("returns only sections for the specified chapter", async () => {
      await repo.saveSection(
        makeSection({ _id: "s_1", chapterId: "chapter_1" }),
      );
      await repo.saveSection(
        makeSection({ _id: "s_2", chapterId: "chapter_2" }),
      );
  ...
  describe("BookRepository.generateId", () => {
    ...
    it("returns unique ids on repeated calls", () => {
      const ids = new Set(
        Array.from({ length: 100 }, () => BookRepository.generateId("x")),
      );
